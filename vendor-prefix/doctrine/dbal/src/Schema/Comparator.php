...
class Comparator
{
    ...
    public static function __callStatic(string $method, array $args): SchemaDiff
    {
        ...
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/4707', 'Calling %s::%s() statically is deprecated.', self::class, $method);
        $comparator = new self();
    ...
    private function doCompareSchemas(Schema $fromSchema, Schema $toSchema)
    {
        ...
        $diff = new SchemaDiff($createdTables, $alteredTables, $droppedTables, $fromSchema, $createdSchemas, $droppedSchemas, $createdSequences, $alteredSequences, $droppedSequences);
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/5770', '%s is deprecated. Use compareTables() instead.', __METHOD__);
    ...
    public function compareTables(Table $fromTable, Table $toTable): TableDiff
    {
        ...
        foreach ($fromTableColumns as $columnName => $column) {
            ...
            $modifiedColumns[$column->getName()] = new ColumnDiff($column->getName(), $toColumn, $changedProperties, $column);
        }
        ...
        foreach ($fromTableIndexes as $indexName => $index) {
            ...
            if ($index->isPrimary() && $toTable->getPrimaryKey() === null || !$index->isPrimary() && !$toTable->hasIndex($indexName)) {
        ...
        return new TableDiff($toTable->getName(), $addedColumns, $modifiedColumns, $droppedColumns, $addedIndexes, $modifiedIndexes, $droppedIndexes, $fromTable, $addedForeignKeys, $modifiedForeignKeys, $droppedForeignKeys, $renamedColumns, $renamedIndexes);
    }
    /**
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/5650', '%s is deprecated. Use diffTable() instead.', __METHOD__);
