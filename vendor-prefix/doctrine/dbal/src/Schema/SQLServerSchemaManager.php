...
use Archetype\Vendor\Doctrine\DBAL\Platforms\SQLServer;
        ...
        if (count($droppedColumns) > 0) {
            ...
            foreach ($droppedColumns as $col) {
                foreach ($this->getColumnConstraints($tableName, $col->getName()) as $constraint) {
                    $this->_conn->executeStatement(sprintf('ALTER TABLE %s DROP CONSTRAINT %s', $tableName, $constraint));
                }
            }
        }
...
SQL;
        ...
        $params = [];
        if ($tableName !== null) {
            $conditions[] = $this->getTableWhereClause($tableName, 'SCHEMA_NAME(f.schema_id)', 'OBJECT_NAME(f.parent_object_id)');
            $sql .= ' WHERE ' . implode(' AND ', $conditions);
        }
