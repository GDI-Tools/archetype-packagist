...
use Archetype\Vendor\Doctrine\DBAL\Driver\PgSQL\Exception\UnexpectedValue;
...
final class Result implements ResultInterface
{
    ...
    public function __construct($result)
    {
        if (!is_resource($result) && !$result instanceof PgSqlResult) {
            throw new TypeError(sprintf('Expected result to be a resource or an instance of %s, got %s.', PgSqlResult::class, is_object($result) ? get_class($result) : gettype($result)));
        }
        $this->result = $result;
    }
    ...
    public function fetchAllNumeric(): array
    {
        ...
        return array_map(fn(array $row) => $this->mapNumericRow($row, $types), $resultSet);
    }
    /** {@inheritDoc} */
    public function fetchAllAssociative(): array
    {
        ...
        return array_map(fn(array $row) => $this->mapAssociativeRow($row, $types), $resultSet);
    }
    /** {@inheritDoc} */
    public function fetchFirstColumn(): array
    {
        ...
        return array_map(fn($value) => $this->mapType($postgresType, $value), pg_fetch_all_columns($this->result));
    }
    ...
    {
        assert($this->result !== null);
        return array_map(fn($value, $field) => $this->mapType($types[$field], $value), $row, array_keys($row));
    }
    /**
