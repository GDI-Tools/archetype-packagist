...
class DB2Platform extends AbstractPlatform
{
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/3263', '%s() is deprecated.', __METHOD__);
        return 254;
    }
    /**
     * {@inheritDoc}
    ...
    {
        $this->doctrineTypeMapping = ['bigint' => Types::BIGINT, 'binary' => Types::BINARY, 'blob' => Types::BLOB, 'character' => Types::STRING, 'clob' => Types::TEXT, 'date' => Types::DATE_MUTABLE, 'decimal' => Types::DECIMAL, 'double' => Types::FLOAT, 'integer' => Types::INTEGER, 'real' => Types::FLOAT, 'smallint' => Types::SMALLINT, 'time' => Types::TIME_MUTABLE, 'timestamp' => Types::DATETIME_MUTABLE, 'varbinary' => Types::BINARY, 'varchar' => Types::STRING];
    }
    /**
     * {@inheritDoc}
     */
    ...
    protected function getVarcharTypeDeclarationSQLSnippet($length, $fixed)
    {
        if ($length <= 0 || func_num_args() > 2 && func_get_arg(2)) {
            Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/3263', 'Relying on the default string column length on IBM DB2 is deprecated' . ', specify the length explicitly.');
        }
    ...
    {
        if ($length <= 0 || func_num_args() > 2 && func_get_arg(2)) {
            Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/3263', 'Relying on the default binary column length on IBM DB2 is deprecated' . ', specify the length explicitly.');
        }
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4749', '%s() is deprecated. Identify platforms by their class.', __METHOD__);
        return 'db2';
    }
    /**
     * {@inheritDoc}
    ...
    {
        return "SELECT NAME FROM SYSIBM.SYSTABLES WHERE TYPE = '" . self::SYSIBM_SYSTABLES_TYPE_TABLE . "'" . ' AND CREATOR = CURRENT_USER';
    }
    /**
     * {@inheritDoc}
     *
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->getAddedColumns() as $column) {
            ...
            $commentsSQL[] = $this->getCommentOnColumnSQL($tableNameSQL, $column->getQuotedName($this), $comment);
        }
        ...
        foreach ($diff->getModifiedColumns() as $columnDiff) {
            ...
            if ($columnDiff->hasCommentChanged()) {
                $commentsSQL[] = $this->getCommentOnColumnSQL($tableNameSQL, $columnDiff->getNewColumn()->getQuotedName($this), $this->getColumnComment($columnDiff->getNewColumn()));
            }
            $this->gatherAlterColumnSQL($tableNameSQL, $columnDiff, $sql, $queryParts);
        }
        ...
        if (!$this->onSchemaAlterTable($diff, $tableSql)) {
            ...
            if ($newName !== \false) {
                ...
                $sql[] = sprintf('RENAME TABLE %s TO %s', $tableNameSQL, $newName->getQuotedName($this));
            }
     ...
     */
    private function gatherAlterColumnSQL(string $table, ColumnDiff $columnDiff, array &$sql, array &$queryParts): void
    {
    ...
    private function getAlterColumnClausesSQL(ColumnDiff $columnDiff): array
    {
        ...
        $clauses = [];
        if ($columnDiff->hasTypeChanged() || $columnDiff->hasLengthChanged() || $columnDiff->hasPrecisionChanged() || $columnDiff->hasScaleChanged() || $columnDiff->hasFixedChanged()) {
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/1519', '%s() is deprecated.', __METHOD__);
        return \true;
    }
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4510', '%s() is deprecated,' . ' use %s::createReservedKeywordsList() instead.', __METHOD__, static::class);
