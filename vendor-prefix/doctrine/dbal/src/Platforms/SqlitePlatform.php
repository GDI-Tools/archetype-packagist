...
use Archetype\Vendor\Doctrine\DBAL\Schema\Constraint;
...
use Archetype\Vendor\Doctrine\DBAL\SQL\Builder\DefaultSelectSQLBuilder;
use Archetype\Vendor\Doctrine\DBAL\SQL\Builder\SelectSQLBuilder;
use Archetype\Vendor\Doctrine\DBAL\TransactionIsolationLevel;
use Archetype\Vendor\Doctrine\DBAL\Types;
use Archetype\Vendor\Doctrine\DBAL\Types\IntegerType;
...
{
    private bool $schemaEmulationEnabled = \true;
    /**
     * {@inheritDoc}
     */
    ...
    public function getLocateExpression($str, $substr, $startPos = \false)
    {
        ...
        return 'CASE WHEN INSTR(SUBSTR(' . $str . ', ' . $startPos . '), ' . $substr . ') > 0 THEN INSTR(SUBSTR(' . $str . ', ' . $startPos . '), ' . $substr . ') + ' . $startPos . ' - 1 ELSE 0 END';
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/1519', 'SqlitePlatform::prefersIdentityColumns() is deprecated.');
        return \true;
    }
    /**
     * {@inheritDoc}
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/5511', '%s is deprecated. Use getSmallIntTypeDeclarationSQL() instead.', __METHOD__);
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/5511', '%s is deprecated. Use getIntegerTypeDeclarationSQL() instead.', __METHOD__);
    ...
    {
        return $this->schemaEmulationEnabled ? str_replace('.', '__', $tableName) : $tableName;
    }
    /**
     * {@inheritDoc}
     *
    ...
    {
        return parent::getForeignKeyDeclarationSQL(new ForeignKeyConstraint($foreignKey->getQuotedLocalColumns($this), $this->emulateSchemaNamespacing($foreignKey->getQuotedForeignTableName($this)), $foreignKey->getQuotedForeignColumns($this), $foreignKey->getName(), $foreignKey->getOptions()));
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/3263', 'SqlitePlatform::getBinaryMaxLength() is deprecated.');
        return 0;
    }
    /**
     * {@inheritDoc}
    ...
    {
        $this->doctrineTypeMapping = ['bigint' => Types\Types::BIGINT, 'bigserial' => Types\Types::BIGINT, 'blob' => Types\Types::BLOB, 'boolean' => Types\Types::BOOLEAN, 'char' => Types\Types::STRING, 'clob' => Types\Types::TEXT, 'date' => Types\Types::DATE_MUTABLE, 'datetime' => Types\Types::DATETIME_MUTABLE, 'decimal' => Types\Types::DECIMAL, 'double' => Types\Types::FLOAT, 'double precision' => Types\Types::FLOAT, 'float' => Types\Types::FLOAT, 'image' => Types\Types::STRING, 'int' => Types\Types::INTEGER, 'integer' => Types\Types::INTEGER, 'longtext' => Types\Types::TEXT, 'longvarchar' => Types\Types::STRING, 'mediumint' => Types\Types::INTEGER, 'mediumtext' => Types\Types::TEXT, 'ntext' => Types\Types::STRING, 'numeric' => Types\Types::DECIMAL, 'nvarchar' => Types\Types::STRING, 'real' => Types\Types::FLOAT, 'serial' => Types\Types::INTEGER, 'smallint' => Types\Types::SMALLINT, 'text' => Types\Types::TEXT, 'time' => Types\Types::TIME_MUTABLE, 'timestamp' => Types\Types::DATETIME_MUTABLE, 'tinyint' => Types\Types::BOOLEAN, 'tinytext' => Types\Types::TEXT, 'varchar' => Types\Types::STRING, 'varchar2' => Types\Types::STRING];
    }
    /**
     * {@inheritDoc}
     *
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->getRenamedColumns() as $oldColumnName => $column) {
            ...
            $columns = $this->replaceColumn($table->getName(), $columns, $oldColumnName, $column);
        ...
        foreach ($diff->getModifiedColumns() as $columnDiff) {
            ...
            $columns = $this->replaceColumn($table->getName(), $columns, $oldColumnName, $columnDiff->getNewColumn());
            if (!isset($newColumnNames[$oldColumnName])) {
                continue;
            }
        ...
        if (!$this->onSchemaAlterTable($diff, $tableSql)) {
            ...
            $newTable = new Table($table->getQuotedName($this), $columns, $this->getPrimaryIndexInAlteredTable($diff, $table), [], $this->getForeignKeysInAlteredTable($diff, $table), $table->getOptions());
    ...
    private function getSimpleAlterTableSQL(TableDiff $diff)
    {
        ...
        if (count($diff->getModifiedColumns()) > 0 || count($diff->getDroppedColumns()) > 0 || count($diff->getRenamedColumns()) > 0 || count($diff->getAddedIndexes()) > 0 || count($diff->getModifiedIndexes()) > 0 || count($diff->getDroppedIndexes()) > 0 || count($diff->getRenamedIndexes()) > 0 || count($diff->getAddedForeignKeys()) > 0 || count($diff->getModifiedForeignKeys()) > 0 || count($diff->getDroppedForeignKeys()) > 0) {
            return \false;
        }
    ...
    private function getIndexesInAlteredTable(TableDiff $diff, Table $fromTable): array
    {
        ...
        foreach (array_merge($diff->getModifiedIndexes(), $diff->getAddedIndexes(), $diff->getRenamedIndexes()) as $index) {
            $indexName = strtolower($index->getName());
