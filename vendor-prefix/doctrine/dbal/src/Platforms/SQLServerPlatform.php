...
use Archetype\Vendor\Doctrine\DBAL\Exception\InvalidLockMode;
use Archetype\Vendor\Doctrine\DBAL\LockMode;
use Archetype\Vendor\Doctrine\DBAL\Platforms\SQLServer\SQL\Builder\SQLServerSelectSQLBuilder;
...
use Archetype\Vendor\Doctrine\DBAL\Schema\Identifier;
...
class SQLServerPlatform extends AbstractPlatform
{
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/1519', 'SQLServerPlatform::prefersIdentityColumns() is deprecated.');
        return \true;
    }
    /**
     * {@inheritDoc}
    ...
    {
        return str_starts_with($possiblyQuotedName, '[') && str_ends_with($possiblyQuotedName, ']') ? substr($possiblyQuotedName, 1, -1) : $possiblyQuotedName;
    }
    /**
    ...
    protected function getCreateColumnCommentSQL($tableName, $columnName, $comment)
    {
        ...
        return $this->getAddExtendedPropertySQL('MS_Description', $comment, 'SCHEMA', $this->quoteStringLiteral($this->unquoteSingleIdentifier($schemaName)), 'TABLE', $this->quoteStringLiteral($this->unquoteSingleIdentifier($tableName)), 'COLUMN', $this->quoteStringLiteral($this->unquoteSingleIdentifier($columnName)));
    }
    /**
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->getAddedColumns() as $column) {
            ...
            if (isset($columnProperties['default'])) {
                $addColumnSql .= ' CONSTRAINT ' . $this->generateDefaultConstraintName($tableName, $column->getQuotedName($this)) . $this->getDefaultValueDeclarationSQL($columnProperties);
            }
            ...
            $commentsSql[] = $this->getCreateColumnCommentSQL($tableName, $column->getQuotedName($this), $comment);
        }
        ...
        foreach ($diff->getModifiedColumns() as $columnDiff) {
            ...
            if ($oldColumn instanceof Column) {
                ...
                if ($hasOldComment && $hasNewComment && $oldComment !== $newComment) {
                    $commentsSql[] = $this->getAlterColumnCommentSQL($tableName, $newColumn->getQuotedName($this), $newComment);
                } elseif ($hasOldComment && !$hasNewComment) {
                    $commentsSql[] = $this->getDropColumnCommentSQL($tableName, $newColumn->getQuotedName($this));
                } elseif (!$hasOldComment && $hasNewComment) {
                    $commentsSql[] = $this->getCreateColumnCommentSQL($tableName, $newColumn->getQuotedName($this), $newComment);
                }
            }
            ...
            $queryParts[] = 'ALTER COLUMN ' . $this->getColumnDeclarationSQL($newColumn->getQuotedName($this), $columnProperties);
            if (!isset($columnProperties['default']) || !$requireDropDefaultConstraint && !$columnDiff->hasDefaultChanged()) {
                continue;
            }
        ...
        foreach ($diff->getRenamedColumns() as $oldColumnName => $newColumn) {
            ...
            $sql[] = "sp_rename '" . $tableNameSQL . '.' . $oldColumnName->getQuotedName($this) . "', '" . $newColumn->getQuotedName($this) . "', 'COLUMN'";
            // Recreate default constraint with new column name if necessary (for future reference).
            if ($newColumn->getDefault() === null) {
                continue;
            }
            $queryParts[] = $this->getAlterTableDropDefaultConstraintClause($tableName, $oldColumnName->getQuotedName($this));
            $queryParts[] = $this->getAlterTableAddDefaultConstraintClause($tableName, $newColumn);
        }
        $tableSql = [];
...
SQL
, $this->generateIdentifierName($oldName), $this->generateIdentifierName($newName), $this->quoteStringLiteral($newName)),
        ];
    }
    /**
    ...
    protected function getAlterColumnCommentSQL($tableName, $columnName, $comment)
    {
        ...
        return $this->getUpdateExtendedPropertySQL('MS_Description', $comment, 'SCHEMA', $this->quoteStringLiteral($this->unquoteSingleIdentifier($schemaName)), 'TABLE', $this->quoteStringLiteral($this->unquoteSingleIdentifier($tableName)), 'COLUMN', $this->quoteStringLiteral($this->unquoteSingleIdentifier($columnName)));
    }
    /**
    ...
    protected function getDropColumnCommentSQL($tableName, $columnName)
    {
        ...
        return $this->getDropExtendedPropertySQL('MS_Description', 'SCHEMA', $this->quoteStringLiteral($this->unquoteSingleIdentifier($schemaName)), 'TABLE', $this->quoteStringLiteral($this->unquoteSingleIdentifier($tableName)), 'COLUMN', $this->quoteStringLiteral($this->unquoteSingleIdentifier($columnName)));
    }
    /**
     * {@inheritDoc}
     */
    ...
    protected function getVarcharTypeDeclarationSQLSnippet($length, $fixed)
    {
        if ($length <= 0 || func_num_args() > 2 && func_get_arg(2)) {
            Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/3263', 'Relying on the default string column length on SQL Server is deprecated' . ', specify the length explicitly.');
        }
        return $fixed ? 'NCHAR(' . ($length > 0 ? $length : 255) . ')' : 'NVARCHAR(' . ($length > 0 ? $length : 255) . ')';
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        if ($length <= 0 || func_num_args() > 2 && func_get_arg(2)) {
            Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/3263', 'Relying on the default binary column length on SQL Server is deprecated' . ', specify the length explicitly.');
        }
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/3263', 'SQLServerPlatform::getBinaryMaxLength() is deprecated.');
        return 8000;
    }
    /**
     * {@inheritDoc}
    ...
    {
        return $this->getAddExtendedPropertySQL('MS_Description', $comment, 'SCHEMA', $this->quoteStringLiteral('dbo'), 'TABLE', $this->quoteStringLiteral($this->unquoteSingleIdentifier($tableName)));
    }
    /** @deprecated The SQL used for schema introspection is an implementation detail and should not be relied upon. */
